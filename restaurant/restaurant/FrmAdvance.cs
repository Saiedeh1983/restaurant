...
namespace restaurant
{
    public partial class FrmAdvance : Form
    {
        ...
        Person p = new Person();
        AdvanceDAL dala = new AdvanceDAL();
        ...
        private void btnfind_Click(object sender, EventArgs e)
        {
            ...
            PersonID = p.PersonID;
            // Fillnumlastsalary();
            DataTable dtpersonList2 = dala.LoadAdvanceList(PersonID);
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (PersonID != 0)
                if (advance.amount != 0)
                {
                    bool Test = dala.InsertAdvance(advance);
                    if (Test == false)
        ...
        private void FillControl(Advance advance)
        {
            id = advance.AdvanceID;
            PersonID = advance.PersonID;
            numlastincrease.Value = Convert.ToDecimal(advance.amount);
            txtDate.Text = advance.Date.Trim();
            txtcouse.Text = advance.cause;
            txtDesc.Text = advance.Desc;
        ...
        private void LoadUC()
        {
            DataTable dtpersonList = dala.LoadAdvanceList(PersonID);
        ...
        private Advance LoadControls()
        {

            if (advance == null)
                advance = new Advance();
            advance.AdvanceID = id;
            advance.PersonID = PersonID;
            advance.amount = Convert.ToInt32(numlastincrease.Value);
            advance.Date = txtDate.Text.Trim();
            advance.cause = txtcouse.Text;
            advance.Desc = txtDesc.Text;
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult dlr;
                dlr = MessageBox.Show("آیا مساعده مورد نظر حذف شود" + "؟", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dlr == DialogResult.Yes)
                {
                    if (advance != null)
                    {
                        dala.DeleteAdvance(advance.AdvanceID, userID);
        ...
        {
            Advance i = new Advance();
            List<Advance> li = new List<Advance>();
            li = dala.LoadAdvanceListWithPersonID(PersonID);
            int c = li.Count;
            int l = 1;
            int result = 0;
            for (; l <= c; l++)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dtpersonListe = dala.LoadAdvanceListFORExcel(userID);
                RKLib.ExportData.Export objExport = new RKLib.ExportData.Export("Win");
                objExport.ExportDetails(dtpersonListe, Export.ExportFormat.Excel, "D:\\AdvanceInfo.xls");
                //txtnumber.Text = "گزارشگيري در مسير D:\\EmployeesInfo.xls با موفقيت انجام شد";
                label8.Text = "گزارشگيري D:\\AdvanceInfo.xls با موفقيت انجام شد";
            }
            catch (Exception Ex)
            {
