...
{
    public partial class FrmHeaithCard : Form
    {
        ...
        Person p = new Person();
        HeaithCardDAL dalr = new HeaithCardDAL();
        HeaithCard heaithCard = new HeaithCard();

        List<HeaithCard> heaithCardeList = new List<HeaithCard>();
        public int id = 0;
        int PersonID = 0;
        int userID = 0;
       public string date = "";
        ...
        private void btnfind_Click(object sender, EventArgs e)
        {
           // Clear();
            ...
            PersonID = p.PersonID;
            // Fillnumlastsalary();
            dtpersonList2 = dalr.LoadHeaithCardList(PersonID);
            gridIncreasesalary.DataSource = dtpersonList2;
            gridMaker();
           // int s = GetYear(txtDate.Text);
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            // PersonID = p.PersonID;
            if (PersonID != 0)
                if (heaithCard.number != "")
                {
                    bool Test = dalr.InsertHeaithCard(heaithCard);
                    if (Test == false)
        ...
        private void Clear()
        {
            ...
            userID = heaithCard.UserID;
            date = heaithCard.expireDate;
            date = string.Empty;
        }
        ...
        private void gridMaker()
        {
            ...
            gridIncreasesalary.Columns[3].HeaderText = "رقم بطاقه الصحه";
        ...
        private void LoadUC()
        {
            dtpersonList = dalr.LoadHeaithCardList(PersonID);
            gridIncreasesalary.DataSource = dtpersonList;
            gridMaker();
            // userID = StaticUnitID.UserID1;
        }
        private HeaithCard LoadControls()
        {

            if (heaithCard == null)
                heaithCard = new HeaithCard();
            heaithCard.HeaithCardID = id;
            heaithCard.PersonID = PersonID;
            heaithCard.expireDate = txtDate.Text.Trim();
            heaithCard.number = txtnumber.Text;
            heaithCard.Desc = txtDesc.Text;
            heaithCard.UserID = userID;
            return heaithCard;
        }
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (heaithCard != null)
            {
                dalr.EditHeaithCard(heaithCard);
        ...
        {
            if (heaithCard.HeaithCardID == 0)
                MessageBox.Show("رکوردی برای حذف شدن وجود ندارد");
            else
            {
                DialogResult dlr;
                dlr = MessageBox.Show("آیا کارت سلامت مورد نظر حذف شود" + "؟", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dlr == DialogResult.Yes)
                {
                    if (heaithCard != null)
                    {
                        dalr.DeleteHeaithCard(heaithCard.HeaithCardID, userID);
        ...
        private void FrmHeaithCard_Load(object sender, EventArgs e)
        {
            List<HeaithCard> LH = dalr.LoadHeaithCardListWithDistinctPersonID(userID);
            HeaithCard hc = new HeaithCard();
            for (int l = 0; LH.Count > l; l++)
            {
                hc.PersonID = LH[l].PersonID;
                List<HeaithCard> lh2 = dalr.LoadHeaithCardList3(LH[l].PersonID);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dtpersonListe = dalr.LoadHeaithCardListFORExcel(userID);
                RKLib.ExportData.Export objExport = new RKLib.ExportData.Export("Win");
                objExport.ExportDetails(dtpersonListe, Export.ExportFormat.Excel, "D:\\HealthCardInfo.xls");
                //txtnumber.Text = "گزارشگيري در مسير D:\\EmployeesInfo.xls با موفقيت انجام شد";
                label3.Text = "گزارشگيري D:\\HealthCardInfo.xls با موفقيت انجام شد";
            }
            catch (Exception Ex)
            {
        ...
        {
            List<HeaithCard> LHh = dalr.LoadHeaithCardListWithDistinctPersonID(userID);
            HeaithCard hc = new HeaithCard();
            for (int l = 0; LHh.Count > l; l++)
            {
                hc.PersonID = LHh[l].PersonID;
                List<HeaithCard> lh2 = dalr.LoadHeaithCardList3(LHh[l].PersonID);
                string max = lh2[0].expireDate;
                if (lh2.Count > 1)
                    for (int l2 = 1; lh2.Count > l2; l2++)
                    {
