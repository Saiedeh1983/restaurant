...
namespace restaurant
{
    ...
    public partial class FrmLabourCard : Form
    {
        ...
        List<LabourCard> residenceList = new List<LabourCard>();
        public int id = 0;
        int PersonID = 0;
        int userID = 0;
        string date = "";
        ...
        private void btnfind_Click(object sender, EventArgs e)
        {
            ...
            PersonID = p.PersonID;
            // Fillnumlastsalary();
            dtpersonList2 = dall.LoadLabourCardList(PersonID);
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            // PersonID = p.PersonID;
            if (PersonID != 0)
                if (labourCard.number != "")
                {
                    bool Test = dall.InsertLabourCard(labourCard);
                    if (Test == false)
        ...
        private void FillControl(LabourCard labourCard)
        {
            id = labourCard.LabourCardID;
            PersonID = labourCard.PersonID;
            txtDate.Text = labourCard.expireDate.Trim();
            txtnumber.Text = labourCard.number;
            txtDesc.Text = labourCard.Desc;
            userID = labourCard.UserID;
            date = labourCard.expireDate;
        }
        ...
        private void gridMaker()
        {
            ...
            gridIncreasesalary.Columns[3].HeaderText = "رقم بطاقه العمل";
        ...
        private void LoadUC()
        {
            dtpersonList = dall.LoadLabourCardList(PersonID);
            gridIncreasesalary.DataSource = dtpersonList;
            gridMaker();
            // userID = StaticUnitID.UserID1;
        }
        private LabourCard LoadControls()
        {

            if (labourCard == null)
                labourCard = new LabourCard();
            labourCard.LabourCardID = id;
            labourCard.PersonID = PersonID;
            labourCard.expireDate = txtDate.Text.Trim();
            labourCard.number = txtnumber.Text;
            labourCard.Desc = txtDesc.Text;
            labourCard.UserID = userID;
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
         ...
         if (labourCard != null)
            {
                dall.EditLabourCard(labourCard);
        ...
        {
            if (labourCard.LabourCardID == 0)
                MessageBox.Show("رکوردی برای حذف شدن وجود ندارد");
            else
            {
                DialogResult dlr;
                dlr = MessageBox.Show("آیا بطاقه العمل مورد نظر حذف شود" + "؟", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dlr == DialogResult.Yes)
                {
                    if (labourCard != null)
                    {
                        dall.DeleteLabourCard(labourCard.LabourCardID, userID);
        ...
        private void FrmLabourCard_Load(object sender, EventArgs e)
        {
        LoadUC();
        }
        ...
        {
            List<LabourCard> LH = dall.LoadLabourCardListWithDistinctPersonID(userID);
            LabourCard hc = new LabourCard();
            for (int l = 0; LH.Count > l; l++)
            {
                hc.PersonID = LH[l].PersonID;
                List<LabourCard> lh2 = dall.LoadLabourCardList3(LH[l].PersonID);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dtpersonListe = dall.LoadLabourCardListFORExcel(userID);
                RKLib.ExportData.Export objExport = new RKLib.ExportData.Export("Win");
                objExport.ExportDetails(dtpersonListe, Export.ExportFormat.Excel, "D:\\LabourCardInfo.xls");
                //txtnumber.Text = "گزارشگيري در مسير D:\\EmployeesInfo.xls با موفقيت انجام شد";
                label3.Text = "گزارشگيري D:\\LabourCardInfo.xls با موفقيت انجام شد";
            }
            catch (Exception Ex)
            {
