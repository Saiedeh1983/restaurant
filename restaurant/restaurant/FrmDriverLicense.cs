...
namespace restaurant
{
    public partial class FrmDriverLicense : Form
    {
        ...
        Person p = new Person();
        DriverLicenseDAL dald = new DriverLicenseDAL();
        DriverLicense driverLicense = new DriverLicense();

        List<DriverLicense> residenceList = new List<DriverLicense>();
        public int id = 0;
        int PersonID = 0;
        int userID = 0;
        string date = "";
        ...
        private void btnfind_Click(object sender, EventArgs e)
        {
            ...
            PersonID = p.PersonID;
            // Fillnumlastsalary();
            dtpersonList2 = dald.LoadDriverLicenseList(PersonID);
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (PersonID != 0)
                ...
                {
                    bool Test = dald.InsertDriverLicense(driverLicense);
                    if (Test == false)
        ...
        private void FillControl(DriverLicense driverLicense)
        {
            id = driverLicense.DriverLicenseID;
            PersonID = driverLicense.PersonID;
            txtDate.Text = driverLicense.expireDate.Trim();
            txtnumber.Text = driverLicense.number;
            txtDesc.Text = driverLicense.Desc;
            userID = driverLicense.UserID;
            date = driverLicense.expireDate;
        }
        ...
        private void gridMaker()
        {
            ...
            gridIncreasesalary.Columns[3].HeaderText = "رقم رخصه القیاده";
        ...
        private void LoadUC()
        {
            dtpersonList = dald.LoadDriverLicenseList(PersonID);
            gridIncreasesalary.DataSource = dtpersonList;
            gridMaker();
            // userID = StaticUnitID.UserID1;
        }
        private DriverLicense LoadControls()
        {

            if (driverLicense == null)
                driverLicense = new DriverLicense();
            driverLicense.DriverLicenseID = id;
            driverLicense.PersonID = PersonID;
            driverLicense.expireDate = txtDate.Text.Trim();
            driverLicense.number = txtnumber.Text;
            driverLicense.Desc = txtDesc.Text;
            driverLicense.UserID = userID;
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (driverLicense != null)
            {
                dald.EditDriverLicense(driverLicense);
        ...
        {
            if (driverLicense.DriverLicenseID == 0)
                MessageBox.Show("رکوردی برای حذف شدن وجود ندارد");
            else
            {
                DialogResult dlr;
                dlr = MessageBox.Show("آیا گواهینامه مورد نظر حذف شود" + "؟", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dlr == DialogResult.Yes)
                {
                    if (driverLicense != null)
                    {
                        dald.DeleteDriverLicense(driverLicense.DriverLicenseID, userID);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dtpersonListe = dald.LoadDriverLicenseListFORExcel(userID);
                RKLib.ExportData.Export objExport = new RKLib.ExportData.Export("Win");
                objExport.ExportDetails(dtpersonListe, Export.ExportFormat.Excel, "D:\\DriverLicenseInfo.xls");
                //txtnumber.Text = "گزارشگيري در مسير D:\\EmployeesInfo.xls با موفقيت انجام شد";
                label3.Text = "گزارشگيري D:\\DriverLicenseInfo.xls با موفقيت انجام شد";
            }
            catch (Exception Ex)
            {
        ...
        {
            List<DriverLicense> LH = dald.LoadDriverLicenseListWithDistinctPersonID(userID);
            DriverLicense hc = new DriverLicense();
            for (int l = 0; LH.Count > l; l++)
            {
                hc.PersonID = LH[l].PersonID;
                List<DriverLicense> lh2 = dald.LoadDriverLicenseList3(LH[l].PersonID);
                ...
                if (lh2.Count > 1)
                    for (int l2 = 1; lh2.Count > l2; l2++)
                    {
                        ...
                        // if (max=<date)
                        {
                            ...
                            MessageBox.Show("گواهینامه" + " " + p2.Name.Trim() + " " + p2.Family.Trim() + " " + "منفضی شده است");
                        }
                    }
                else if (lh2.Count == 1)
                    if (GetmaxDate(t1, max) == true)
